...
export interface IRequirePlugin<T = string> {
    (module: string, require: IRequire, isRelease: boolean): T;
}

const clearPath = (path: string) => path.replace(/^\//, '').replace(/\?.+/, '');

const fileFormat = (prefix: string, ext: string): IRequirePlugin<void | string> => {
    return (module: string, require: IRequire, isRelease: boolean): void | string => {
...
export const css: IRequirePlugin<void | string> = fileFormat('css!', '.css');
 ...
 */
export const cssTheme: IRequirePlugin<void | string> = fileFormat('css!theme?', '_');

export const wml: IRequirePlugin<void | string> = fileFormat('wml!', '.wml');

export const tmpl: IRequirePlugin<void | string> = fileFormat('tmpl!', '.tmpl');
 ...
 */
export const pathPlugins: IRequirePlugin<void | string>[] = [
    json, cssTheme, css, wml, tmpl, text, i18n, cdn, js
];
