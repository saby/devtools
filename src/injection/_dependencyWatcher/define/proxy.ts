...
export let proxyDefine = (define: IDefine, storage: ModuleStorage, logger: ILogger): IDefine => {
    ...
    return new Proxy(define, {
        apply(
            ...
            if (!needReplaceDependencies(dependencies, replacedModules)) {
                return define.call(_this, name, dependencies, (...depModules: unknown[]) => {
                    _init(name);
            ...
            return define.call(_this, name, dependencies, (...depModules: unknown[]) => {
                _init(name);
                let _depModules: unknown[] = replaceDependencies({
                    proxyModules,
                    args: depModules,
                    dependencies,
                    moduleName: name
